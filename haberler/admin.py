...
from .models import (
    ...
    KoseYazari, KoseYazisi, Yorum, Destekci, Siir
)

# =========================================================
# 📸 1. INSTAGRAM POST OLUŞTURMA MOTORU
# =========================================================
...
# --- (EKSİK OLAN KISIM EKLENDİ) ECZANE LİNKLERİ ---
@admin.register(EczaneLinki)
class EczaneLinkiAdmin(admin.ModelAdmin):
    list_display = ('ilce_adi', 'url')
    list_editable = ('url',)

# admin.py içinde KoseYazariAdmin sınıfını bul ve bunu yapıştır:

@admin.register(KoseYazari)
class KoseYazariAdmin(admin.ModelAdmin):
    # Listede adı, başyazar durumu ve aktifliği görünsün
    list_display = ('ad_soyad', 'basyazar_mi', 'aktif_mi')
    
    # Listeye girmeden direkt tik atıp kaydedebilmek için:
...
class YorumAdmin(admin.ModelAdmin):
    ...
    def yorumlari_onayla(self, request, queryset): queryset.update(aktif=True)
    yorumlari_onayla.short_description = "Seçili yorumları onayla"
    def govde_kisalt(self, obj): return obj.govde[:50] + "..." if len(obj.govde) > 50 else obj.govde
...
class SiirAdmin(admin.ModelAdmin):
    ...
    search_fields = ('baslik', 'sair', 'siir_metni')
